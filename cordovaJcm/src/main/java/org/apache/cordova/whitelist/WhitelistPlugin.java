...
public class WhitelistPlugin extends CordovaPlugin {
    ...
    public WhitelistPlugin(Context context) {
        ...
        //wpl2018/5/28添加从intent中获取config文件名称
        Intent intent = context instanceof Activity ? ((Activity)context).getIntent() : null;
        String configName;
        if (intent == null){
            configName = "config";
        }else{
            configName = intent.getExtras().getString("configName","config");
        }
        new CustomConfigXmlParser().parse(context,configName);
    }
    ...
    public void pluginInitialize() {
        if (allowedNavigations == null) {
            ...
            allowedRequests = new Whitelist();
            //wpl2018/5/28添加 从intent中获取config文件名称
            Intent intent = webView.getContext() instanceof Activity ? ((Activity) webView.getContext()).getIntent() : null;
            String configName;
            ...
            new CustomConfigXmlParser().parse(webView.getContext(),configName);
        }
    }
